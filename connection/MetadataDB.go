...
// ConnectMetadata returns an available database connection from the pool
func ConnectMetadataOLD() (*gorm.DB, error) {
	if len(dbs) == 0 {
		err := initializeDbs()
		if err != nil {
...
func initializeHosts() ([]string, string, error) {
	dsn, ok := os.LookupEnv("METADATA_CATALOGUE_CONNECTION_STRING")
	log.Println("METADATA_CATALOGUE_CONNECTION_STRING:", dsn)
	if !ok {
		return nil, "", fmt.Errorf("METADATA_CATALOGUE_CONNECTION_STRING is not set")
	}
